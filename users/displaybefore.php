...
$approve = "No";
$sql = "SELECT * FROM post WHERE approve='$approve' AND jname='$myname' ORDER BY postid DESC";
$result = $db->query($sql);
...
if(isset($_POST['change1'])){
    ...
    if (count($errors) == 0) {
        ...
        $resetcommment = "Waiting for review";
        $resetname = "";
        $query = "UPDATE post SET title='$title', body='$body', img='$image', comment='$resetcommment',ename='$resetname' WHERE postid='$pid'";
